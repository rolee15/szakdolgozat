...
const api = {
    async getCharacters(): Promise<KanaCharacter[]> {
      const response = await apiFetch(`${API_KANA_URL}/katakana`);
      if (!response.ok) throw new Error('Failed to fetch characters');
      return response.json();
    },

    async getCharacterDetail(character: string): Promise<KanaCharacter> {
      const response = await apiFetch(`${API_KANA_URL}/katakana/${character}`);
      if (!response.ok) throw new Error('Failed to fetch character details');
      return response.json();
    },

    async getExamples(character: string): Promise<Example[]> {
      const response = await apiFetch(`${API_KANA_URL}/katakana/${character}/examples`);
