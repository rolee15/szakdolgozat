...
public class HiraganaController : ControllerBase
{
    ...
    public HiraganaController(IKanaService kanaService)
    {
    ...
    {
        IEnumerable<KanaDto> characters = await _kanaService.GetKanaCharacters(KanaType.Hiragana, GetUserId());
    ...
    {
        KanaDetailDto charDetail = await _kanaService.GetCharacterDetail(character, KanaType.Hiragana, GetUserId());
    ...
    {
        IEnumerable<KanaExampleDto> examples = await _kanaService.GetExamples(character, KanaType.Hiragana);
        return Ok(examples);
    }
}
